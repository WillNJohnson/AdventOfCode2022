...
def doNProcessThrows(N, monkey):
    common_modulus = prod([monkey[n]['divisible'] for n in range(len(monkey))])
    for _ in range(N):
        processThrow(monkey, common_modulus)
    return monkey

def processThrow(monkey, common_modulus):
    new = 0
    for n in range(len(monkey)):
        starting_items = monkey[n]['starting']
        while starting_items != []:
            old = monkey[n]['starting'][0]
            new = eval(monkey[n]['op']) % common_modulus
...
with open('data.txt', 'r') as f:
    ...
    for line in f:
        ...
        if data == []:
            monkey[n] = {'starting': starting_items, 'op': operation, 'divisible': test_divisibility_by, 'on_true': on_true_throw_to, 'on_false': on_false_throw_to, 'inspections': 0, 'state': 0}
            n += 1
...
monkey = doNProcessThrows(10000, monkey)

# get top 2 number of inspections
top_two_inspections = sorted([monkey[n]['inspections'] for n in monkey])[-2:]

print(f'The level of monkey business after 10000 rounds is {prod(top_two_inspections)}.')
